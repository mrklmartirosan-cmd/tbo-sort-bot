...
def _norm_company(s):
    """'ТОО \"L-TRADING\"' / 'Индивидуальный предприниматель X' -> единый ключ для сравнения.
    Отбрасывает и полные юр.формы, и сокращения — чтобы разные написания одного контрагента совпадали."""
    s = str(s).lower()
    s = re.sub(r"[\"'«»“”]", " ", s)
    for full in ("индивидуальный предприниматель", "товарищество с ограниченной ответственностью",
                 "акционерное общество", "крестьянское хозяйство", "частное предприятие",
                 "общество с ограниченной ответственностью"):
        s = s.replace(full, " ")
    s = re.sub(r"\b(тоо|ао|ип|оао|зао|ооо|llp|llc|кх|чп)\b", " ", s)
