...
BOT_TOKEN = "8301597645:AAH1YI80SUG0439UJTHqyw8jhsPfNydgWrg"
...
Распознай все данные и верни ТОЛЬКО JSON (без пояснений, без markdown) в таком формате:
{
  "дата": "дд.мм.гггг",
  "фио": "ФИО оператора",
  "вес_шин": число,
  ...
  "фракция_6_8": число,
  "металл_корд": число,
  "примечание": "текст или пустая строка"
}
Если какое-то значение не читается — поставь 0. Дату бери из документа, если не видна — сегодняшнюю."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📸 Отправь фото ежедневного отчёта — я распознаю данные и сохраню в таблицу.\n\n"
        "📊 Команды:\n"
        "/start — приветствие\n"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reply = (
            ...
            f"_Если что-то распознано неверно — напиши мне_"
        )
        ...
        await update.message.reply_text(
            ...
            "• Сделать фото чётче\n"
            "• Убедиться что всё в кадре\n"
            "• Хорошее освещение"
        )

async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📸 Отправь фото отчёта — я распознаю и сохраню данные.\n"
        "Или используй /get чтобы скачать таблицу."
    )
